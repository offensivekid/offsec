...
app.post('/api/register', authLimiter, (req, res) => {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO users (username, email, password_hash, has_private_access, created_at)
      VALUES (?, ?, ?, ?, ?)
    `).run(sanitize(username), sanitize(email), hash, registrationKey ? 1 : 0, Date.now());
...
app.get('/api/me', requireAuth, (req, res) => {
  const user = db.prepare('SELECT id, username, is_admin, has_private_access, email FROM users WHERE id = ?')
